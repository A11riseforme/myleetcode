...
void dfs(int* candidates, int candidatesSize, int target, int** result, int** columnSizes, int* tmp, int size, int pos, int *totalSize) {
    ...
    for (int i = pos; i < candidatesSize; ++i) {
        ...
        dfs(candidates, candidatesSize, target - candidates[i], result, columnSizes, tmp, size + 1, i, totalSize);
    }
    return;
}
...
int main(int argc, char** argv) {
    ...
    */
    ...
    int *columnSizes;
    int returnSize;
    int **result = combinationSum(nums, sizeof(nums) / sizeof(nums[0]), target, &columnSizes, &returnSize);
